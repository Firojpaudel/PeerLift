...
export default async function Dashboard() {
  ...
  // Fetch user with all related data — cast to `any` to avoid 
  // complex union-type issues when merging mentor/learner sessions.
  ...
  const skillsOffered: any[] = user.skillsOffered || [];
  const skillsWanted: any[] = user.skillsWanted || [];
  const sessionsAsMentor: any[] = user.sessionsAsMentor || [];
  const sessionsAsLearner: any[] = user.sessionsAsLearner || [];
  const requestsReceived: any[] = user.requestsReceived || [];

  // A user is "new" if they have no skills and no sessions
  const isNewUser = skillsOffered.length === 0 && 
  ...
  const allSessions: any[] = [
    ...sessionsAsMentor.map((s: any) => ({ ...s, role: 'mentor' })),
    ...sessionsAsLearner.map((s: any) => ({ ...s, role: 'learner' }))
              ...
              {requestsReceived.length > 0 && (
                <section className="mb-6">
                  ...
                  <div className="space-y-3">
                    {requestsReceived.map((req: any) => (
                      <div key={req.id} className="bg-bg-elevated rounded-2xl border border-border p-5 shadow-[var(--shadow-sm)] flex items-center justify-between hover:border-primary-300 transition-colors">
                        <div className="flex items-center gap-4">
                          <Link href={`/u/${req.sender?.username}`} className="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center shrink-0 border-2 border-bg-elevated relative hover:scale-105 transition-transform dark:bg-primary-900/30 group overflow-hidden">
                            {req.sender?.avatarUrl ? (
                              <img src={req.sender.avatarUrl} alt={req.sender.name} className="w-full h-full object-cover" />
                            ) : (
                              <span className="text-primary-700 font-bold dark:text-primary-400 group-hover:scale-110 transition-transform">{req.sender?.name?.charAt(0) || "?"}</span>
                            )}
                          </Link>
                          <div>
                            <Link href={`/u/${req.sender?.username}`} className="group">
                              <h4 className="font-bold text-text-primary text-[15px] flex items-center gap-1.5 group-hover:text-primary-600 transition-colors">
                                {req.sender?.name || "Unknown"} <span className="inline-block font-mono text-[10px] font-bold bg-primary-100 text-primary-700 px-2 py-0.5 rounded-full dark:bg-primary-900/30 dark:text-primary-400">PENDING</span>
                              </h4>
                            </Link>
                ...
                <section>
                  ...
                  <div className="space-y-3">
                    {recentHistory.map((s: any) => (
                      <div key={s.id} className="flex items-center gap-4 p-4 bg-bg-elevated rounded-xl border border-border shadow-sm hover:border-primary-300 transition-colors">
                        <div className={`w-10 h-10 rounded-full ${s.role === 'mentor' ? 'bg-green-100 text-green-600' : 'bg-amber-100 text-amber-600'} dark:bg-opacity-20 flex items-center justify-center shrink-0`}>
                          <span className="font-bold text-lg">{s.role === 'mentor' ? '+' : '-'}</span>
                        </div>
                        <div className="flex-1">
                          <h4 className="font-bold text-text-primary text-[15px]">
                            ...
                            {' with '}
                            {s.role === 'mentor' ? (s.learner?.name || 'a peer') : (s.mentor?.name || 'a peer')}
                          </h4>
                          <p className="text-sm text-text-secondary">Completed {s.duration} min session</p>
                        </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="w-full bg-bg-secondary h-2 rounded-full mt-1">
                    <div className="bg-primary-500 h-2 rounded-full" style={{ width: `${Math.min(sessionsAsMentor.length * 10, 100)}%` }}></div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 mt-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="w-full bg-bg-secondary h-2 rounded-full mt-1">
                    <div className="bg-secondary-500 h-2 rounded-full" style={{ width: `${Math.min(sessionsAsLearner.length * 10, 100)}%` }}></div>
                  </div>
                </div>
              </div>
            </div>
